// P3: MÚLTIPLES IMPLEMENTACIONES DE POPCOUNT
// Marco Girela Vida 
// 4-11-2023
...
unsigned int popcount5(unsigned * array, size_t tam)
{
	...
	for (size_t i = 0; i < tam; i++)
	{
		...
		{
			val += x & 0x01010101U;	// "Por cada byte de x, comprueba si su LSB es 1 y si lo es, suma 1 al byte equivalente en val"
			x >>= 1;				// "Mueve x un bit a la derecha (y por tanto mueve cada byte de x un bit a la derecha)"
		}							// Repite 8 veces, es decir, hasta que se haya recorrido cada byte
									// Al final val tendrá almcenado en cada byte el popcount de cada byte equivalente en x
									// (ten en cuenta que un bit no puede tener más de 8 unos, por lo que ningún byte de val valdrá más de 8)
		val += (val >> 16);			// Sumamos el resultado de los 2 primeros bytes de val con los dos últimos y almacenamos el resultado en los dos primeros
		val += (val >> 8);			// Sumamos el resultado del segundo byte (=popcount(byte4) + popcount(byte2)) con el primer byte (=popcount(byte3)+popcount(byte1)) = sumatoria de todos los popcounts
									// Observa que un int (4 bytes = 32bits) no podrá tener más de 32 unos, por lo que el resultado del popcount cabrá en 5 bits, nosotros le damos 8 bits (1byte), asi que cabe de sobra
		result += val & 0xFF;		// Borramos toda la "basura" del resto de bytes y nos quedamos solo con el primero, que es donde está el resultados
	}
...
// No entiendo nada de lo que se hace en esta implementación
...
{
	#if TEST==0 || TEST==4
		for (size_t i = 0; i < SIZE; i++)	lista[i]=i;		//Inicializa el array
